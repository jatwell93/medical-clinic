...
PHASE2_ASSUMPTIONS_KEYS = [
    ...
    '    "sa1_shapefile":          "SA1_2021_VIC_GDA2020.gpkg",  # VIC-only GeoPackage — pre-filtered from ABS ASGS Ed 3 (68% smaller upload)',
    '    "poa_shapefile":          "POA_2021_VIC_GDA2020.gpkg",  # VIC-only GeoPackage — pre-filtered from ABS ASGS Ed 3 (74% smaller upload)',
...
def catchment_cells():
    ...
    cells.append(code(
        ...
        'SA1_ZIP_FALLBACK = PROJECT_ROOT / "data" / "local" / "SA1_2021_AUST_GDA2020.zip"',
        'POA_ZIP_FALLBACK = PROJECT_ROOT / "data" / "local" / "POA_2021_AUST_GDA2020_SHP.zip"',
        "",
        'if not SA1_PATH.exists() and not SA1_ZIP_FALLBACK.exists():',
        '    print(f"⚠ SA1 boundary file missing: tried {SA1_PATH.name} and {SA1_ZIP_FALLBACK.name}")',
        ...
        "    USE_SA1 = True",
        "",
        '# POA: prefer GPKG (VIC-only, no filter needed); fall back to ZIP + postcode-prefix filter',
        "if POA_PATH.exists():",
        '    poa = gpd.read_file(POA_PATH).to_crs("EPSG:7855")  # already VIC-only',
        "elif POA_ZIP_FALLBACK.exists():",
        '    print(f"[geom] GPKG not found — using full ZIP {POA_ZIP_FALLBACK.name} (slower upload/load)")',
        "    poa = gpd.read_file(POA_ZIP_FALLBACK)",
        '    poa = poa[poa["POA_CODE21"].str.startswith("3")].to_crs("EPSG:7855")  # VIC postcodes 3000-3999',
        "else:",
        '    raise FileNotFoundError(f"POA boundary file not found: {POA_PATH} or {POA_ZIP_FALLBACK}")',
        'print(f"[geom] POA: {len(poa)} VIC polygons, CRS={poa.crs.to_epsg()}")',
        "",
        "if USE_SA1:",
        "    if SA1_PATH.exists():",
        '        sa1 = gpd.read_file(SA1_PATH).to_crs("EPSG:7855")  # already VIC-only',
        "    else:",
        "        sa1 = gpd.read_file(SA1_ZIP_FALLBACK)",
