...
**Domain:** Australian healthcare location analytics — geospatial catchment analysis, ABS Census API, Google Places competitor mapping, GP clinic P&L modelling, delivered as a free-Colab notebook + executive PDF
...
**Critical: the base URL changed on 29 Nov 2024.** `https://api.data.abs.gov.au/data/...` is the OLD pattern (the one in PROJECT.md). The current pattern is:

```
https://data.api.abs.gov.au/rest/data/{dataflowId}/{dataKey}?format=csvfilewithlabels
```
...
**Dataflow IDs confirmed to exist for 2021 Census POA data:**
...
**Recommended access approach: raw `requests` + `format=csvfilewithlabels` → `pandas.read_csv`.** Rationale:
...
**Prescribed workflow per dataflow:**
1. GET the datastructure once (`/rest/datastructure/ABS/C21_G01_POA?references=codelist`), cache it, and print the dimension order (typically something like `{CHARACTERISTIC}.{REGION}` for Census flows — **verify at runtime; do not hardcode dimension order from this doc**).
2. Build dataKey with POA codes, e.g. `...3067+3066+3121+3068+3070+3078+3079+3101+3122+3123...` (`+` = OR within a dimension, `.` separates dimensions, empty = all).
3. GET data with `format=csvfilewithlabels` through the `requests_cache` session.
4. On any HTTP failure or timeout: log a visible warning and fall back to the local GCP DataPack CSV/xlsx.

No API key required; it is rate-limited and still labelled Beta — cache everything, and keep the local DataPack fallback as a first-class code path, not an afterthought.
...
**Recommendation: build the executive PDF as a separate artifact, not a notebook export.** jinja2 HTML template + CSS + `weasyprint.HTML(string=html).write_pdf("executive_summary.pdf")`, with matplotlib/contextily figures embedded as base64 PNGs. Rationale:
...
Fallback / secondary option for a "full notebook" PDF for technical appendix: `jupyter nbconvert --to webpdf` (`pip install "nbconvert[webpdf]" && playwright install chromium`) — works in Colab, avoids the multi-GB LaTeX install of `--to pdf`. Or `colab2pdf`/Quarto if a one-click notebook PDF is wanted. Treat this as optional.
...
if IN_COLAB:
    ...
    GOOGLE_PLACES_KEY = userdata.get("GOOGLE_PLACES_KEY")   # user adds via 🔑 Secrets sidebar
    PROJECT_DIR = Path("/content/drive/MyDrive/medical-clinic")  # after drive.mount
else:
    from dotenv import load_dotenv
    load_dotenv()                                            # reads .env next to the notebook
    GOOGLE_PLACES_KEY = os.getenv("GOOGLE_PLACES_KEY")
    PROJECT_DIR = Path(__file__).resolve().parent if "__file__" in dir() else Path.cwd()

DATA_DIR, CACHE_DIR = PROJECT_DIR / "data", PROJECT_DIR / "data" / "cache"
CACHE_DIR.mkdir(parents=True, exist_ok=True)
assert GOOGLE_PLACES_KEY, "Set GOOGLE_PLACES_KEY in Colab Secrets or .env"
```
...
pip install "geopandas>=1.1,<2" "shapely>=2.1" "pyproj>=3.7" "folium>=0.20" ^
    ...
    weasyprint python-dotenv numpy-financial openpyxl
```
...
**If ABS API is down or rate-limits during a run:**
...
**If weasyprint install fails locally on Windows (GTK dependency):**
...
**If a Nearby Search query returns exactly 20 results (saturation):**
...
---
*Stack research for: Australian healthcare location analytics (Colab feasibility study)*
*Researched: 2026-07-05*
